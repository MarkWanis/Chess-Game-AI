'''
Another AI idea:
Create a function that goes through every singe move of the AI's color on the board and evaluates the board value based on what's threatening its pieces and maybe on what it's threatening - MOSTLY DONE

Create a condition where if AI piece is threatening a piece and it isn't threatened itself, increase the board value - DONE

Add a point for each available move of the color to encourage pieces to move to more open spots (Ex: Opening move e2 to e4 should be worth more because it allows the queen and the bishop a large amount of options) - DONE

Create temporary variables to call instead of having to call functions multiple times - DONE

As of now, the AI will not threaten the king with a guarded piece that only the king can kill. If we implement that the AI can threaten the king even if it is being threatened by the king (and only the king) then we can get more checkmates.

Make it so it encourages the AI to threaten another piece if its value higher than the AI's piece (Idk if this is already in there to some effect) - NOT DONE
'''
...
virtual_board = [
['black    rook', 'black  knight', 'black  bishop', 'black   queen', 'black    king', 'black  bishop', 'black  knight', 'black    rook'], ['black    pawn', 'black    pawn', 'black    pawn', 'black    pawn', 'black    pawn', 'black    pawn', 'black    pawn', 'black    pawn'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['white    pawn', 'white    pawn', 'white    pawn', 'white    pawn', 'white    pawn', 'white    pawn', 'white    pawn', 'white    pawn'], ['white    rook', 'white  knight', 'white  bishop', 'white   queen', 'white    king', 'white  bishop', 'white  knight', 'white    rook']]
#virtual_board = [['black    king', 'black  bishop', 'white    rook', '-------------', '-------------', '-------------', '-------------', '-------------'], ['black    pawn', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', 'white   queen', '-------------', '-------------', 'black    pawn', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------'], ['-------------', '-------------', '-------------', '-------------', 'white    king', '-------------', '-------------', '-------------']]
...
def move_brings_into_check(position, new_position, color): #Check if move brings into check by creating temp board and moving the piece temporarily and using check_check - Boolian function
  temp_virtual_board = set_temp_board()

  move_brings_into_check = False

  virtual_board[new_position[0]][new_position[1]] = virtual_board[position[0]][position[1]] #This part moves the piece
  virtual_board[position[0]][position[1]] = '-------------'

  if check_check(color):
    move_brings_into_check = True

  reset_board(temp_virtual_board)

  return move_brings_into_check
...
def set_temp_board():
  ...
  return temp_virtual_board


def reset_board(temp_virtual_board):
...
def ai_choose_move(color): #Third AI  -  Right now the AI is selecting the first possible move from left to right top to bottom 
  temp_virtual_board = set_temp_board()
  possible_moves = find_piece_possible_moves(find_pieces(color), color)
...
def find_old_position(piece_positions, new_position):
  for element in piece_positions:
...
def ai_choose_move_easy(color):
  piece_positions = find_pieces(color)
  possible_moves = find_piece_possible_moves_list(piece_positions, color)
  ...
  for element in piece_positions:
    if selected_move in find_piece_type([element[0], element[1]], color): 
      old_position = [element[0], element[1]]
      
  return [old_position, selected_move] 
...
while True:
  if virtual_board[7] == ['-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------', '-------------']:
    print("All hail Waillord - Jweezy")
  ...
  if player_turn == "white":
    ...
    while True:
      ...
      if move_brings_into_check(user_move, new_user_move, player_turn):
        print("That move will bring you into a check! Please select a different move.")
        continue
    ...
    ai_move = ai_choose_move(player_turn)

    move_piece(ai_move[0], ai_move[1])

    promote_pawn(player_turn)

    print_board(ai_move[0], ai_move[1])
